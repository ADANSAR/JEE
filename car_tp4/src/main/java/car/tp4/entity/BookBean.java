...
public class BookBean {
    ...
    /**
     * @param ID identifiant du livre à retourner
     *
    ...
    /**
     * @param id identifiant du livre concerné
     * @param number quantité à enlever
     *
    ...
    /**
     * @param id identifiant du livre concerné
     * @param number quantité à rajouter
     *
	...
	public List<Book> findTitle(String title) {
		 ...
		 * @return liste des livres triés par rapport à leurs date de parution
		 */
		 public List<Book> getBookByAuteur(String auteur) {
	         Query query = entityManager.createQuery("SELECT m from Book m");
	         List<Book> findedBookList = query.getResultList();
	         ArrayList<Book> findedBookAuteur = new ArrayList<Book>();
	         if (auteur != null) {
	             String auteurSearch = Normalizer.normalize(auteur.toLowerCase(), Normalizer.Form.NFD).replaceAll("\\s", "").replaceAll("[^\\p{ASCII}]", "");
	             for (Book book : findedBookList) {
	                 if (Normalizer.normalize(book.getAuthor().toLowerCase(), Normalizer.Form.NFD).replaceAll("\\s", "").replaceAll("[^\\p{ASCII}]", "").contains(auteurSearch)) {
	                     findedBookAuteur.add(book);
	                 }
	             }
	         }
	 		return findedBookAuteur;
	 	}
}
